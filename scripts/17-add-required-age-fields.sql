...
-- First ensure minimum_age exists for all categories (in case some don't have it)
...
SELECT 
  ...
  'Minimum Age',
  'number',
  -- Set field_order to be after existing fields (or 0 if no fields exist)
...
-- Then add maximum_age field to categories that don't have it yet
INSERT INTO category_fields (category_id, field_name, field_label, field_type, field_order, is_required, validation_rules, options, placeholder, help_text)
SELECT 
  c.id,
  'maximum_age',
  'Maximum Age',
  'number',
  -- Set field_order to be after existing fields (including the minimum_age we just added)
  COALESCE((SELECT MAX(field_order) + 1 FROM category_fields WHERE category_id = c.id), 1),
  false,
  '{"min": 0, "max": 150}',
  '[]',
  'e.g., 99',
