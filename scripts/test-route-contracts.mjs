...
assert.match(marketplaceSearch, /marketplace_search_attractions_v6/)
...
assert.match(filterDefinitionsRoute, /subcategoryProduct/)
...
assert.match(marketplaceSearchMigration, /definition\.subcategory_id = property\.subcategory_id/)
...
assert.match(marketplaceSearchMigration, /catalog_visible boolean not null default true/)
assert.match(marketplaceSearchMigration, /product_attribute_definitions[\s\S]*subcategory_id/)
assert.match(marketplaceSearchMigration, /ticketing_add_organizer_attraction_v3/)
assert.match(marketplaceSearchMigration, /Selected activity does not belong to the selected category/)

const categoryDataMigration = await source("supabase/migrations/20260922173100_migrate_category_wrappers.sql")
assert.match(categoryDataMigration, /'Adrenalina', 'adrenalina'/)
assert.match(categoryDataMigration, /'Dzieci i rodzina', 'dzieci-i-rodzina'/)
assert.match(categoryDataMigration, /'Zagadki i gry', 'zagadki-i-gry'/)
assert.match(categoryDataMigration, /set category_id = adrenalina, subcategory_id = paintball/)
assert.match(categoryDataMigration, /catalog_visible = false/)
assert.match(categoryDataMigration, /catalog_enforce_category_activity_pair/)

const discoveryLoader = await source("lib/marketplace/discovery.ts")
assert.match(discoveryLoader, /marketplace_search_attractions_v6/)
...
assert.match(attractionsView, /MARKETPLACE_SEARCH_PARAM_KEYS/)
assert.match(attractionsView, /marketplaceSearchResetUpdates\(\["categories"\]\)/)
assert.match(attractionsView, /onClearFilters=\{clearFilters\}/)
assert.match(attractionsView, /location: next\.bbox \? next\.location\?\.trim\(\) \|\| null : null/)
assert.match(attractionsView, /bbox: next\.bbox \|\| null/)
assert.match(attractionsView, /focusBbox=\{filters\.bbox\}/)
assert.match(attractionsView, /searchParams\.get\("location"\)/)
assert.match(attractionsView, /Moja lokalizacja/)
...
assert.match(attractionFilters, /createDefaultFilterState/)
assert.match(attractionFilters, /onClearFilters\(createDefaultFilterState\(\)\)/)
assert.match(attractionFilters, /resolveCurrentLocation\(getMapTilerKey\(\)\)/)
assert.match(attractionFilters, /bbox: result\.bbox/)
assert.match(attractionFilters, /LocationAutocomplete/)
assert.match(attractionFilters, /place\.bbox/)
assert.match(attractionFilters, /Użyj mojej lokalizacji/)
...
assert.match(searchDialog, /buildCategoryCatalog/)
assert.match(searchDialog, /\.eq\("catalog_visible", true\)/)
assert.doesNotMatch(searchDialog, /CATEGORY_GROUPS|fallbackCategories/)
...
assert.match(searchDialog, /marketplaceSearchResetUpdates\(\)/)
assert.match(searchDialog, /navigateToResults: false/)
assert.match(searchDialog, /\[isOpen, urlSearchString\]/)
assert.match(searchDialog, /\[groupedCategories, isOpen, selectedCategories\]/)
assert.doesNotMatch(searchDialog, /\[isOpen, groupedCategories\]/)
...
assert.match(searchDialog, /resolveCurrentLocation\(getMapTilerKey\(\)\)/)
assert.match(searchDialog, /LocationAutocomplete/)
assert.match(searchDialog, /locationMode !== "text" && locationBbox/)
assert.match(searchDialog, /location: locationMode !== "text"/)
assert.match(searchDialog, /place\.bbox/)
assert.match(searchDialog, /Użyj mojej lokalizacji/)

const urlState = await source("lib/search/url-state.ts")
assert.match(urlState, /MARKETPLACE_SEARCH_PARAM_KEYS/)
assert.match(urlState, /"location"/)
assert.match(urlState, /"bbox"/)
assert.match(urlState, /marketplaceSearchResetUpdates/)
assert.match(urlState, /navigateToResults\?: boolean/)
assert.match(urlState, /clearTimeout\(debounceTimerRef\.current\)/)
assert.match(urlState, /searchString: searchParams\.toString\(\)/)

const currentLocation = await source("lib/search/current-location.ts")
assert.match(currentLocation, /navigator\.geolocation\.getCurrentPosition/)
assert.match(currentLocation, /DEFAULT_LOCATION_RADIUS_KM/)
assert.match(currentLocation, /reverseGeocode/)
assert.match(currentLocation, /bboxAroundCoordinates/)

const geocoding = await source("lib/maps/geocoding.ts")
assert.match(geocoding, /DEFAULT_LOCATION_RADIUS_KM = 30/)
assert.match(geocoding, /api\.maptiler\.com\/geocoding/)
assert.match(geocoding, /autocomplete/)
assert.match(geocoding, /fuzzyMatch/)
assert.match(geocoding, /countryCode\.toLowerCase/)
assert.match(geocoding, /searchPlaces/)
assert.match(geocoding, /reverseGeocode/)
assert.match(geocoding, /geocodeAddress/)

const locationAutocomplete = await source("components/location-autocomplete.tsx")
assert.match(locationAutocomplete, /searchPlaces/)
assert.match(locationAutocomplete, /getMapTilerKey/)
assert.match(locationAutocomplete, /250/)
assert.match(locationAutocomplete, /countryCode = "PL"/)

const topNav = await source("components/top-nav.tsx")
assert.match(topNav, /nearbyFilter/)
assert.match(topNav, /geographicLocation/)
assert.match(topNav, /searchParams\.get\("location"\)/)
assert.match(topNav, /Moja lokalizacja/)
...
assert.match(categoryBar, /buildCategoryCatalog/)
assert.match(categoryBar, /\.eq\("catalog_visible", true\)/)
...
assert.match(categoryBar, /handleSubcategorySelect[\s\S]*navigate\(slug \|\| selectedCategoryData\?\.slug/)
assert.doesNotMatch(categoryBar, /CATEGORY_GROUPS|FALLBACK_CATEGORIES/)

const activityCategorySelect = await source("components/activity-category-select.tsx")
assert.match(activityCategorySelect, /subcategories/)
assert.match(activityCategorySelect, /Rodzaj atrakcji/)
assert.doesNotMatch(activityCategorySelect, /CATEGORY_GROUPS|categoryGroup/)
...
assert.match(attractionMap, /applyEnjoyHubMapTheme\(instance, "enjoyhub"\)/)
assert.match(attractionMap, /GeolocateControl/)
assert.match(attractionMap, /showUserLocation: true/)
assert.match(attractionMap, /focusBbox/)
assert.match(attractionMap, /fittedFocusBboxRef/)
assert.doesNotMatch(attractionMap, /Prosta PL|enjoyhub-map-theme|handleMapThemeChange/)
...
assert.match(locationPicker, /applyEnjoyHubMapTheme\(map, "simple"\)/)
...
assert.match(mapLibreLoader, /MAPTILER_BASE_STYLE = "base-v4"/)
assert.match(mapLibreLoader, /name:pl/)
assert.match(mapLibreLoader, /applyEnjoyHubMapTheme/)
assert.match(mapLibreLoader, /#dcefc1/)
assert.match(mapLibreLoader, /#b9deea/)
assert.match(mapLibreLoader, /#f3f2ed/)
...
assert.match(organizerOnboardingUi, /<ActivityCategorySelect/)
assert.match(organizerOnboardingUi, /subcategories=\{subcategories\}/)
assert.match(organizerOnboardingUi, /LocationAutocomplete/)
assert.match(organizerOnboardingUi, /cityCenter/)
