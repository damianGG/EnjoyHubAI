...
const route = "/api/cron/demand-notifications"
...
export async function GET(request: Request) {
  ...
  return runMonitoredCron({
    slug: "enjoyhub-demand-notifications",
    schedule: "15 7 * * *",
    route,
    ...
    if (error) {
      reportServerError(error, {
        area: "demand",
        operation: "prepare_notifications",
        route,
        requestId,
    ...
    if (isStripeConnectEnabled && deliveries.length > 0) {
      ...
      if (accountError) {
        reportServerError(accountError, {
          area: "demand",
          operation: "payment_readiness_lookup",
          route,
          requestId,
          extras: { organizationCount: organizationIds.length },
        })
    ...
    for (const delivery of deliveries) {
      if (isStripeConnectEnabled && !connectAccountReady(paymentAccounts.get(delivery.organization_id))) {
        ...
        if (deferError) {
          failed += 1
          reportServerError(deferError, {
            area: "demand",
            operation: "defer_notification",
            route,
            requestId,
            extras: { notificationId: delivery.notification_id },
          })
        }
        continue
      }
      ...
      if (result.sent) {
        ...
        if (completionError) {
          failed += 1
          reportServerError(completionError, {
            area: "demand",
            operation: "repair_sent_state",
            route,
            requestId,
            extras: { notificationId: delivery.notification_id },
          })
      ...
      if (completionError) {
        reportServerError(completionError, {
          area: "demand",
          operation: "failure_state_update",
          route,
          requestId,
          extras: { notificationId: delivery.notification_id },
        })
      ...
      failed += 1
      reportServerError(new Error(result.error ?? result.reason ?? "Demand availability email could not be queued"), {
        area: "demand",
        operation: "queue_notification_email",
        route,
        requestId,
        extras: {
          notificationId: delivery.notification_id,
