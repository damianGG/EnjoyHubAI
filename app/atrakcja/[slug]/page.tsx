...
import { extractPublicAttractionCode } from "@/lib/utils"
...
export async function generateMetadata({ params }: Pick<AttractionPageProps, "params">): Promise<Metadata> {
  ...
  const attraction = await getPublicAttractionSeoRecordByCode(extractPublicAttractionCode(slug))
...
export default async function AttractionPage({ params }: AttractionPageProps) {
  ...
  const { slug } = await params
  const publicCode = extractPublicAttractionCode(slug)
  const attraction = await getPublicAttractionSeoRecordByCode(publicCode)

  if (!attraction) notFound()

  const id = attraction.id
  const ticketingVenue = await getMarketplaceTicketingVenue(id)

  const canonicalPath = getAttractionCanonicalPath(attraction)
  if (`/atrakcja/${slug}` !== canonicalPath) permanentRedirect(canonicalPath)
