...
export default async function AdminDashboard() {
  ...
  const { data: organizations } = await supabase.rpc("platform_admin_list_organizations", { p_search: null, p_limit: 250 })
  const orgs = (organizations ?? []) as OrganizationRow[]

  const canSeeFinance = role === "platform_superadmin" || role === "platform_finance"
  const canSeeAudit = role === "platform_superadmin" || role === "platform_support" || role === "platform_finance"
  const revenue = canSeeFinance ? orgs.reduce((sum, organization) => sum + Number(organization.confirmed_revenue), 0) : null
  ...
  let audit: any[] = []
  if (canSeeAudit) {
    const { data } = await supabase.rpc("platform_admin_list_audit", { p_organization_id: null, p_limit: 8 })
    audit = data ?? []
  }
    ...
    <main className="container mx-auto max-w-7xl px-4 py-8">
      <div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-muted-foreground">Organizacje, użytkownicy, weryfikacja i wsparcie właścicieli z zakresem dopasowanym do roli administratora.</p>
        </div>
      ...
      <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <Metric icon={CreditCard} label="Potwierdzony obrót" value={revenue === null ? "—" : formatMoney(revenue, "PLN")} />
      </div>

      <div className={canSeeAudit ? "grid gap-6 lg:grid-cols-[1.35fr_0.65fr]" : "grid gap-6"}>
        <Card>
          <CardHeader>
            <CardTitle>Organizacje wymagające uwagi</CardTitle>
            <CardDescription>Weryfikacja, aktywacja płatności i kompletność konfiguracji.</CardDescription>
          </CardHeader>
            ...
            {orgs.length === 0 ? (
              ...
              <Link key={organization.organization_id} href={`/admin/organizacje/${organization.organization_id}`} className="flex items-center justify-between gap-4 py-4 hover:text-primary">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{organization.venue_count} obiektów · {organization.attraction_count} atrakcji · {organization.offer_count} ofert</p>
                </div>
