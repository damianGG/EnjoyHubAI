...
import { signUp, signInWithGoogle, signInWithFacebook, signInWithInstagram } from "@/lib/actions"
  ...
  return (
    <Button type="submit" variant="outline" disabled={pending} className="w-full bg-transparent">
      {pending ? (
        ...
        <>
          <svg className="mr-2 h-4 w-4" viewBox="0 0 24 24" fill="url(#instagram-gradient-signup)">
            <defs>
              <linearGradient id="instagram-gradient-signup" x1="0%" y1="100%" x2="100%" y2="0%">
...
export default function SignUpForm({ inline = false, onSuccess, onSwitchToLogin }: SignUpFormProps = {}) {
  ...
  const content = (
    <>
      ...
      <form action={signInWithFacebook} className="mb-2">
        <FacebookSignInButton />
      </form>

      <form action={signInWithInstagram} className="mb-4">
