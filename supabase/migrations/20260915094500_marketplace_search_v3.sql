-- EnjoyHub marketplace discovery search v3
--
-- Unifies every user-facing discovery filter in one database operation and
-- stops treating the legacy accommodation-era price_per_night column as a
-- ticket price. Unknown-price content profiles remain discoverable, but only
-- canonical live ticket prices participate in price filtering/sorting/display.

create or replace function public.marketplace_search_attractions_v3(
...
property_base as (
  select
    ...
    case
      ...
      when lower(property.title) = params.query_text then 120
      when lower(property.title) like params.query_text || '%' then 110
      when lower(property.city) = params.query_text then 100
      when lower(property.city) like params.query_text || '%' then 90
      when lower(property.title) like '%' || params.query_text || '%' then 80
      when lower(property.city) like '%' || params.query_text || '%' then 70
      when lower(coalesce(category.name, '')) like '%' || params.query_text || '%' then 60
      when lower(coalesce(subcategory.name, '')) like '%' || params.query_text || '%' then 55
  ...
  where property.is_active = true
    ...
    and (
      coalesce(cardinality(p_property_types), 0) = 0
      or property.property_type = any(p_property_types)
    )
    and (
      coalesce(cardinality(p_amenities), 0) = 0
      or coalesce(property.amenities, array[]::text[]) @> p_amenities
    )
    and (
      p_guests is null
      or property.max_guests >= p_guests
    )
    and (
...
enriched as (
  select
    ...
    (session.property_id is not null) as has_online_sales
...
revoke all on function public.marketplace_search_attractions_v3(
...
grant execute on function public.marketplace_search_attractions_v3(
  text, text[], text[], text[], integer,
  numeric, numeric, numeric, numeric,
  integer, integer, date, date, timestamptz, boolean,
  numeric, numeric, text, integer, integer
...
comment on function public.marketplace_search_attractions_v3(
  text, text[], text[], text[], integer,
  numeric, numeric, numeric, numeric,
  integer, integer, date, date, timestamptz, boolean,
  numeric, numeric, text, integer, integer
) is 'Server-only discovery RPC. All user filters, canonical ticket prices, ranking and pagination are applied before results are returned.';
