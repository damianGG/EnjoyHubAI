-- Finalize the separation between legacy profile metadata and the canonical
-- platform/organizer authorization models.
...
begin
  ...
  -- Do not mutate public.users.is_host or public.users.role here. Organizer
  -- authorization is exclusively organization-membership based.
...
) is 'Atomically creates organizer membership, public attraction, ticketing venue, offer, prices, schedule and initial sessions without relying on legacy profile roles.';

-- Legacy offer tables still had RLS bypasses tied to public.users.role =
-- super_admin. Replace them with the canonical platform staff role.
drop policy if exists "Super admins can manage all offers" on public.offers;
create policy "Platform superadmins can manage all offers"
on public.offers for all to authenticated
using (public.platform_is_staff(array['platform_superadmin']::public.platform_staff_role[]))
with check (public.platform_is_staff(array['platform_superadmin']::public.platform_staff_role[]));

drop policy if exists "Super admins can manage all offer availability" on public.offer_availability;
create policy "Platform superadmins can manage all offer availability"
on public.offer_availability for all to authenticated
using (public.platform_is_staff(array['platform_superadmin']::public.platform_staff_role[]))
with check (public.platform_is_staff(array['platform_superadmin']::public.platform_staff_role[]));

drop policy if exists "Super admins can manage all offer bookings" on public.offer_bookings;
create policy "Platform superadmins can manage all offer bookings"
on public.offer_bookings for all to authenticated
using (public.platform_is_staff(array['platform_superadmin']::public.platform_staff_role[]))
with check (public.platform_is_staff(array['platform_superadmin']::public.platform_staff_role[]));