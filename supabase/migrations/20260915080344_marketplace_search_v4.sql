-- EnjoyHub marketplace discovery search v4
--
-- Final discovery contract: taxonomy comes from categories/subcategories,
-- party-size availability comes from sellable ticketing sessions, and Polish
-- text search is accent-insensitive without relying on legacy accommodation
-- fields.

create or replace function public.marketplace_search_attractions_v4(
...
with search_params as (
  select
    nullif(
      translate(lower(btrim(p_query)), 'ąćęłńóśźż', 'acelnoszz'),
      ''
    ) as query_text,
...
property_base as (
  ...
  where (
      ...
      or not exists (
        ...
        where token.value <> ''
          and property.search_text not like '%' || token.value || '%'
      )
    )
    and (
    ...
    and (
      ...
      or property.amenities @> p_amenities
    )
    and (
...
session_candidates as (
  ...
  where session.starts_at > now()
    ...
    and (
      p_guests is null
      or greatest(session.capacity - coalesce(inventory.reserved_capacity, 0), 0) >= p_guests
    )
    and (
      (
...
enriched as (
  ...
  where (p_age_min is null or coalesce(age.maximum_age, 150) >= p_age_min)
    ...
    and (
      p_guests is null
      or sellable.property_id is null
      or session.property_id is not null
    )
    and (
...
revoke all on function public.marketplace_search_attractions_v4(
...
grant execute on function public.marketplace_search_attractions_v4(
  text, text[], text[], text[], integer,
  numeric, numeric, numeric, numeric,
  integer, integer, date, date, timestamptz, boolean,
  numeric, numeric, text, integer, integer
...
comment on function public.marketplace_search_attractions_v4(
  text, text[], text[], text[], integer,
  numeric, numeric, numeric, numeric,
  integer, integer, date, date, timestamptz, boolean,
  numeric, numeric, text, integer, integer
) is 'Server-only marketplace discovery RPC using canonical attraction taxonomy and live ticketing capacity.';
